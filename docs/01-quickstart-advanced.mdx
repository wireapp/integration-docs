...
Follow [the app creation guide](02-manage-apps/01-create-app.mdx) and get application ID and API Token to identify and authenticate your app.
...
|-------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `applicationId` + `apiToken`  | Universally unique identifier (UUID) of the app and a token that allows access to Wire-server endpoints. <br /> Available in Team Management after [registering an app](./02-manage-apps/01-create-app.mdx) |
...
public class Main {
    public static void main(String[] args) {
        final var wireAppSdk = new WireAppSdk(
            ...
            "YOUR_API_TOKEN",
            "YOUR_API_HOST",
            "yourGeneratedSecureKeyByteArray!".getBytes(),
...
- In case you need to build the SDK locally, you can skip the signing option by running: `./gradlew publishToMavenLocal -PskipSigning=true` (**_Keep in mind_** to include `repositories { mavenLocal() }` in your `build.gradle.kts` file in the app where you use the locally built SDK)
