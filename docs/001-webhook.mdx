...
- A **Jira or ServiceNow** webhook that posts a notification when a ticket opens or changes priority.
- A **CI/CD pipeline** (GitHub Actions, Jenkins, GitLab) that alerts a team channel on a build failure or deployment.
- A **monitoring tool** (PagerDuty, Grafana, Datadog) that fires an alert when a service degrades.
- A **Zendesk or Salesforce** event that routes a new support request to the right Wire conversation.
...
sequenceDiagram
    ...
    App->>App: Parse and format payload
...
The simplest pattern: when an event fires, post a formatted message to a known conversation. Otherwise, set the conversationId as part of the webhook URL or data to also have routing.
...
See [sending messages](./03-developer-interface/02-interactions/01-send-message.mdx) for how to compose composite messages with action buttons, and [on button clicked](./03-developer-interface/01-events/06-on-button-clicked.mdx) for handling the response.
