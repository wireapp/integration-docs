...
The `getUser` method returns a `WireUser` object containing:
...
val user: WireUser = applicationManager.getUserSuspending(userId)
...
WireUser user = applicationManager.getUser(userId);
...
const userId = new QualifiedId('user-id', 'user-domain.com')

const user: WireUser = await applicationManager.getUser(userId)
...
public void onTextMessageReceived(WireMessage.Text message) {
    WireUser senderInfo = manager.getUser(message.sender());
...
public override async onTextMessageReceived(wireMessage: TextMessage): Promise<void> {
  const senderInfo = await applicationManager.getUser(wireMessage.sender)
  console.log(`Message from ${senderInfo.name}: ${wireMessage.text}`)
}
```
