...
```typescript
public override async onMessageDeleted(wireMessage: DeletedMessage): Promise<void> {
    if (!wireMessage.sender) return

    const user = await this.manager.getUser(wireMessage.sender)
