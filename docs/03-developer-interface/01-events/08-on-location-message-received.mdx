...
Along with the [Base, Ephemeral, and Replyable properties](01-message-model.md),
...
public override async onLocationMessageReceived(wireMessage: Location): Promise<void> {
    // Wire HQ in Berlin
    const hqLat = 52.5240008
    const hqLon = 13.4025010

    // User location
    const lat = wireMessage.latitude
    const lon = wireMessage.longitude

    // Simple approximate distance (in km)
    const latDiff = lat - hqLat
    const lonDiff = (lon - hqLon) * Math.cos((((lat + hqLat) / 2) * Math.PI) / 180)
    const distanceKm = Math.sqrt(latDiff * latDiff + lonDiff * lonDiff) * 111.0
